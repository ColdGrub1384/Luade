...
class TemplatesViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        var newURL: URL
        if templateURL.lastPathComponent == "Share Sheet.lua" {
            newURL = sharedScriptsURL.appendingPathComponent("Example.lua")
        } else if let iCloudDrive = DocumentBrowserViewController.iCloudContainerURL {
        ...
        while FileManager.default.fileExists(atPath: newURL.path) {
            newURL = newURL.deletingLastPathComponent().appendingPathComponent("\(name) \(i).lua")
